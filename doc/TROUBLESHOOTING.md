...
| BOOT-button reset doesn't seem to work | The default GPIO 0 is the devkit's BOOT button, but it must be held during runtime (not at reset/power-on). Hold past **30 seconds without releasing** to wipe creds; releasing any time between 3 and 30 s toggles the OTA gate instead. The 30 s wipe threshold is deliberately long since the action is destructive. The serial log narrates both transitions. |
